...
// --- VENTAS: detalle POR FECHA en un rango (cada venta con su fecha y monto) ---
app.get('/api/almacenes/ventas-rango', async (req, res) => {
  try {
    const ini = String(req.query.fecha_inicio || '').trim();
    const fin = String(req.query.fecha_fin || '').trim();
    if (!ini || !fin) return res.json([]);
    ...
    diaSnap.docs.forEach(d => {
      const a = d.data();
      const cant = parseFloat(a.total_ventas) || 0;
      if (cant <= 0) return;
      out.push({
        fecha: a.fecha,
        item_id: Number(a.item_id),
...
const COLS_RENOMBRAR = [
  ...
  { col: 'cocina_stock_diario', campo: 'ingrediente' },
  { col: 'cocina_ventas', campo: 'nombre' },
  { col: 'cocina_receta_ingredientes', campo: 'ingrediente' },
  { col: 'cocina_compras', campo: 'nombre' },
  { col: 'compras', campo: 'nombre' },
  { col: 'porcionamientos', campo: 'nombre' },
  { col: 'receta_ingredientes', campo: 'ingrediente' },
  { col: 'ventas', campo: 'nombre' },
  { col: 'eventos_compras', campo: 'nombre' },
  { col: 'limpieza_compras', campo: 'nombre' },
  { col: 'eventos_stock', campo: 'ingrediente' },
  { col: 'limpieza_stock', campo: 'ingrediente' },
  { col: 'eventos_movimientos', campo: 'ingrediente' },
  { col: 'limpieza_movimientos', campo: 'ingrediente' },

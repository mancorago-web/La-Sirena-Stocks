...
function verVariacionPrecios() {
  return api('GET', '/api/compras/variacion').then(conVariacion => {
    ...
    const filas = conVariacion.map(x => {
      ...
      const up = dif > 0;
      const fAnt = x.anterior_fecha ? ' <span style="font-weight:400;font-size:0.75rem;color:#aaa;">(' + esc(x.anterior_fecha) + (x.anterior_cantidad ? ' ×' + x.anterior_cantidad : '') + ')</span>' : '';
      const fUlt = x.ultimo_fecha ? ' <span style="font-weight:400;font-size:0.75rem;color:#aaa;">(' + esc(x.ultimo_fecha) + (x.ultimo_cantidad ? ' ×' + x.ultimo_cantidad : '') + ')</span>' : '';
...
function filtrarVariacion(term) {
  ...
  document.querySelectorAll('#variacion-precios-body tr').forEach(tr => {
    ...
    if (q) {
      ...
      if (!n.includes(q)) ok = false;
    }
    if (ok && (ini || fin)) {
      const uf = tr.getAttribute('data-ult-fecha') || '';
      if (ini && (!uf || uf < ini)) ok = false;
      if (fin && (!uf || uf > fin)) ok = false;
    }
...
function cargarSalidas(fechaIni, fechaFin) {
  if (!fechaIni) fechaIni = document.getElementById('fecha-salidas').value;
  if (!fechaFin) fechaFin = document.getElementById('fecha-salidas-fin')?.value || fechaIni;
  if (!fechaIni) return;
  const btnGuardar = document.getElementById('tab-salidas')?.querySelector('.btn-guardar-dia');
  ...
  if (fechaFin > fechaIni) {
    ...
    api('GET', '/api/almacenes/con-inventario-rango?fecha_inicio=' + encodeURIComponent(fechaIni) + '&fecha_fin=' + encodeURIComponent(fechaFin)).then(data => {
      renderRangoDetalle(container, data, 'salidas');
    }).catch(e => console.error(e));
    return;
  }
  if (btnGuardar) { btnGuardar.disabled = false; btnGuardar.textContent = '💾 GUARDAR SALIDAS'; }
...
function verDetallesSalidas() {
  const fecha = document.getElementById('fecha-salidas').value;
  if (!fecha) { alert('Selecciona una fecha'); return; }
  const fin = document.getElementById('fecha-salidas-fin')?.value || fecha;
  if (fin > fecha) {
    api('GET', '/api/almacenes/con-inventario-rango?fecha_inicio=' + encodeURIComponent(fecha) + '&fecha_fin=' + encodeURIComponent(fin)).then(data => {
      const body = document.getElementById('modal-body');
      body.innerHTML = '<h3>Detalle de Salidas — ' + fecha + ' a ' + fin + '</h3>';
      const wrap = document.createElement('div');
      wrap.style.marginTop = '0.5rem';
      renderRangoDetalle(wrap, data, 'salidas');
...
function verDetallesVentas() {
  ...
  if (fin > fecha) {
    api('GET', '/api/almacenes/ventas-rango?fecha_inicio=' + encodeURIComponent(fecha) + '&fecha_fin=' + encodeURIComponent(fin)).then(list => {
      const body = document.getElementById('modal-body');
      body.innerHTML = '<h3>Detalle de Ventas — ' + fecha + ' a ' + fin + '</h3>';
      const wrap = document.createElement('div');
      wrap.style.marginTop = '0.5rem';
      renderVentasRango(wrap, list, fecha, fin);
      body.appendChild(wrap);
      document.getElementById('modal').style.display = 'block';
    });
    return;
...
function cargarVentas(fechaIni, fechaFin) {
  ...
  if (fechaFin > fechaIni) {
    if (btnGuardar) { btnGuardar.disabled = true; btnGuardar.textContent = '🔒 SOLO LECTURA (RANGO)'; }
    api('GET', '/api/almacenes/ventas-rango?fecha_inicio=' + encodeURIComponent(fechaIni) + '&fecha_fin=' + encodeURIComponent(fechaFin)).then(list => {
      renderVentasRango(container, list, fechaIni, fechaFin);
    }).catch(e => console.error(e));
    return;
  }
...
function cargarIngresos(fechaIni, fechaFin) {
  if (!fechaIni) fechaIni = document.getElementById('fecha-ingresos').value;
  if (!fechaFin) fechaFin = document.getElementById('fecha-ingresos-fin')?.value || fechaIni;
  if (!fechaIni) return;
  const btnGuardar = document.getElementById('tab-ingresos')?.querySelector('.btn-guardar-dia');
  ...
  if (btnGuardar) { btnGuardar.disabled = false; btnGuardar.textContent = '💾 GUARDAR INGRESOS'; }
  getInventario(fechaIni).then(data => {
    const alNombres = {};
    (data || []).forEach(al => { alNombres[al.id] = al.nombre; });
    data = (data || []).filter(a => a.id !== 3 && a.id !== 9 && a.id !== 16);
...
// Render del detalle de VENTAS por fecha (rango): muestra la FECHA de cada venta y permite filtrar
// por item con el buscador (las filas llevan data-item-id y el item en la primera celda).
function renderVentasRango(container, list, ini, fin) {
  if (!container) return;
  if (!list || !list.length) { container.innerHTML = '<p style="color:#888;">No hay ventas registradas en el rango <b>' + ini + ' a ' + fin + '</b>.</p>'; return; }
  const total = list.reduce((s, r) => s + (r.cantidad || 0), 0);
  const t = (u) => u ? new Date(u).toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit' }) : '—';
  container.innerHTML = '<div class="table-wrap"><table><thead><tr><th>Item</th><th>Fecha</th><th>Almacén</th><th>Cantidad</th><th>Hora</th><th>Usuario</th></tr></thead><tbody>'
    + list.map(r => '<tr data-item-id="' + r.item_id + '"><td>' + esc(r.nombre) + '</td><td>' + esc(r.fecha) + '</td><td>' + esc(r.almacen) + '</td><td>' + fmt3(r.cantidad) + '</td><td>' + t(r.updated_at) + '</td><td>' + esc(DISPLAY_NAMES[r.saved_by] || r.saved_by || '-') + '</td></tr>').join('')
    + '</tbody></table></div>'
    + '<p style="font-weight:700;color:#0f3460;margin-top:0.5rem;">TOTAL VENTAS: <b>' + fmt3(total) + '</b> (' + list.length + ' registros)</p>';
}
...
function selectUnidades(id, valor, opciones) {
  ...
  let opts = '';
  if (v && !opciones.some(o => String(o).toLowerCase() === v.toLowerCase())) {
    opts += '<option value="' + esc(v) + '">' + esc(v) + '</option>';
  }
  opts += opciones.map(u => '<option value="' + esc(u) + '" ' + (String(u).toLowerCase() === v.toLowerCase() ? 'selected' : '') + '>' + esc(u) + '</option>').join('');
  return '<select id="' + id + '" style="width:100%;padding:0.4rem;border:1px solid #ccc;border-radius:4px;margin-top:0.3rem;">' + opts + '</select>';
}
...
function agregarItemBaseDatos() {
  ...
  body.innerHTML = '<h3>AGREGAR ITEM A LA BASE DE DATOS</h3>'
    ...
    + '<label style="display:block;margin-bottom:0.4rem;">Unidad Compra: ' + selectUnidades('bd-add-uc', 'unidad', _UNIDADES_COMPRA) + '</label>'
    ...
    + '<label style="display:block;margin-bottom:0.4rem;">Unidad Venta: ' + selectUnidades('bd-add-uv', 'unidad', _UNIDADES_VENTA) + '</label>'
...
function editarItemBaseDatos(origen, id) {
  ...
  body.innerHTML = '<h3>EDITAR ITEM (' + x.zona + ')</h3>'
    ...
    + '<label>Unidad Venta: ' + selectUnidades('bd-edit-uv', x.unidad_venta, _UNIDADES_VENTA) + '</label>'
...
function enviarFaltantesWhatsApp() {
  ...
  api('GET', '/api/reportes/diferencias?fecha_inicio=' + ini + '&fecha_fin=' + fin).then(data => {
    ...
    let msg = 'ITEM FALTANTES - ' + fechaTxt + '\n\n';
    conFalta.forEach((it, idx) => { msg += (idx + 1) + '. ' + it.nombre + ' - ' + it.falta_almacen + '\n'; });
...
function verStockBarraGeneral() {
  ...
  const CATEGORIAS = [
    ...
    { label: 'LICORES', test: n => /LICOR|BARNIDET|BARDINET|AMARETTO|AMARETTTO|DISSARONO|DRAMBUIE|KAHLUA|KWAIFEH|GRAN KAFA|OURI|MOLINARI|FRANGELICO|BAILEYS|MALIBU|SAKE|SHO CHIKU BAI|GRANADINA|MATACUY|BULL DOG|AVELINO|CACHAZA|CONTREAU|ANIS/i.test(n) },
    { label: 'VERMOUTHS Y APERITIVOS', test: n => /VERMOUTH|VERMOTH|VERMUT|CINZANO|CAMPARI|APEROL|FERNET|BRANCA/i.test(n) },
...
function mostrarResultadoConteo(r, fecha, ajustado, volver) {
  ...
  window._conteoInformeData = { fecha, difs };
  body.innerHTML = `
...
function enviarInformeWhatsApp(fecha) {
  ...
  // Agrupar por mueble; dentro de cada mueble FALTANTES luego SOBRANTES
  const grupos = {};
  difs.forEach(d => {
    const g = String(d.grupo || 'SIN MUEBLE').toUpperCase();
  ...
  const ordenarAlpha = (a, b) => String(a.ingrediente).localeCompare(String(b.ingrediente), 'es');
  const muebleOrder = Object.keys(grupos).sort((a, b) => {
    if (a === 'COMPRAS DIARIAS') return 1;
    if (b === 'COMPRAS DIARIAS') return -1;
    return a.localeCompare(b, 'es');
  });
  ...
  api('GET', '/api/cocina/stock/con-inventario?fecha=' + encodeURIComponent(fecha)).then(grupos => {
    ...
    function fila(i) {
      ...
      return `<tr data-cocina-id="${i.id}" data-cantidad="${i.cantidad}" data-unidad="${esc(i.unidad)}" data-familia="${esc(i.familia)}" data-subgrupo="${esc(i.subgrupo || '')}">
        ...
        <td style="white-space:nowrap">
          <button class="btn-precio-cocina" onclick="editarPrecioCocina(${i.id})" title="Poner/editar el precio unitario" style="background:#e65100;color:#fff;border:none;padding:0.2rem 0.4rem;border-radius:3px;cursor:pointer;font-size:0.75rem;">💰</button>
  ...
  api('GET', '/api/compras/detalle?fecha_inicio=' + encodeURIComponent(fechaIni) + '&fecha_fin=' + encodeURIComponent(fechaFin)).then(list => {
    ...
    const render = (anteriores) => {
      ...
      if (_compararCompras) {
        ...
        // Agrupado por PROVEEDOR en cajas con borde negro. Cada caja usa los MISMOS anchos fijos
        // de columna (table-layout:fixed) para que todo quede alineado entre proveedores.
        const gruposProv = {};
        ...
        const COLGROUP_COMPRAS = '<col style="width:36%"><col style="width:9%"><col style="width:10%"><col style="width:11%"><col style="width:12%"><col style="width:9%"><col style="width:70px">';
        const provOrder = Object.keys(gruposProv).sort((a, b) => {
        ...
        contenido = provOrder.map(prov => {
          ...
          zonas.forEach(z => {
            ...
            cuerpo += `<div style="border-top:1px dashed #ddd;padding:0.45rem 0 0.25rem 0;">
              ...
              <div class="table-wrap"><table style="width:100%;table-layout:fixed;font-size:0.78rem;"><colgroup>${COLGROUP_COMPRAS}</colgroup>
...
const ORDEN_CATEGORIAS_COCINA = ['FRUTAS', 'VERDURAS', 'PESCADO - BRUTO', 'PESCADO PORC. - BARRA FRIA', 'PESCADO PORC. - BARRA CALIENTE', 'CARNE', 'POLLO', 'LACTEOS', 'ABARROTES', 'ACEITES', 'SEMILLAS', 'RECETAS BASE', 'LIMPIEZA', 'VINOS', 'CERVEZAS', 'OTROS'];

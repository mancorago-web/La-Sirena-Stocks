...
function cargarRecetas(openId) {
  api('GET', '/api/recetas').then(data => {
    ...
    const ordenCat = ['RECETAS BASE', 'Clásicos', 'Mojitos', 'Limonadas', 'SODAS', 'JUGO DE FRUTAS', 'DEL BARMAN', 'Chilcanos y Sours', 'SHOTS', 'VINO TINTOS'];
      let html = '';
  ...
  Promise.all([
    ...
    let html = `
      ...
      <select id="edit-receta-categoria" style="width:100%;margin-bottom:1rem;">
        ${['RECETAS BASE','Clásicos','Mojitos','Limonadas','SODAS','JUGO DE FRUTAS','DEL BARMAN','Chilcanos y Sours','SHOTS','VINO TINTOS'].map(c =>
